...
$query = "SELECT * FROM PRODUCT WHERE SHARED = '1'";
...
echo "<table border =1><tr><th>Name</th><th>Description</th><th>Price</th><th>Available Stock</th><th>Image</th><th>Amend</th><th>Delete</th></tr>";
	while ($row = mysqli_fetch_assoc($result)){
						...
						echo "<tr><td>" . $row["PRODUCT_NAME"]. 
						      ...
						      "</td><td>" . '<a href="delicatessen/amendproduct.php?id='. $row['PRODUCT_ID'].'">Amend</a>'.
						      "</td><td>" . '<a href="delicatessen/delete.php?id='. $row['PRODUCT_ID'].'">Delete</a>'.
						      "</td></tr> " ;
...
{    
    ...
    if(empty($name) || empty($lastname) || empty($phonenumber) || empty($email) || empty($address) || empty($postcode)|| empty($password)) {            
        ...
        //updating the table
        $result = mysqli_query($connection, "SELECT * FROM PRODUCT WHERE SHARED = '1'");
...
<h3 class="table-links"> Add A New Product </h3>
    <form name="form1" method="post" action="delicatessen/insertproduct.php">
        <table border="0">
            <tr> 
                <td>Product Name</td>
            ...
            <tr> 
                <td>Trader ID</td>
                <td><input type="text" name="traderid" placeholder="2005"></td>
            </tr>
            <tr> 
                <td>Shared</td>
                <td><input type="text" name="shared" placeholder="1"></td>
            </tr>
            <tr>
