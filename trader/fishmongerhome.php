...
$query = "SELECT * FROM PRODUCT WHERE PRODUCT_TYPE = 'Seafood'";
...
echo "<table border =1><tr><th>Name</th><th>Description</th><th>Price</th><th>Available Stock</th><th>Image</th><th>Amend</th><th>Delete</th></tr>";
	while ($row = mysqli_fetch_assoc($result)){
						...
						echo "<tr><td>" . $row["PRODUCT_NAME"]. 
						      ...
						      "</td><td>" . '<a href="fishmonger/amendproduct.php?id='. $row['PRODUCT_ID'].'">Amend</a>'.
						      "</td><td>" . '<a href="fishmonger/delete.php?id='. $row['PRODUCT_ID'].'">Delete</a>'.
						      "</td></tr> " ;
...
{    
    ...
    if(empty($name) || empty($lastname) || empty($phonenumber) || empty($email) || empty($address) || empty($postcode)|| empty($password)) {            
        ...
        //updating the table
        $result = mysqli_query($connection, "SELECT * FROM PRODUCT WHERE PRODUCT_TYPE = 'Seafood'");
...
<h3 class="table-links"> Add A New Product </h3>
    <form name="form1" method="post" action="fishmonger/insertproduct.php">
        <table border="0">
            <tr> 
                <td>Product Name</td>
            ...
            <tr> 
                <td>Product Type</td>
                <td><input type="text" name="producttype" placeholder = "Seafood"></td>
            </tr>
            <tr> 
            ...
            <tr> 
                <td>Trader ID</td>
                <td><input type="text" name="traderid" placeholder="2003"></td>
            </tr>
            <tr> 
                <td>Shared</td>
